...
def inv3(m):
    ...
    adjm[]
'''
def lineCurvature2D(Vertces,Lines):
    '''
    ...
    for i in range(Na.shape[0]):
        Disa[i]=-length(vertices[i],vertices[Na[i]])
        Disb[i]=length(vertices[i],vertices[Nb[i]])

    x=np.zeros([vertices.shape[0],3])
    y=np.zeros([vertices.shape[0],3])
    for i in range(vertices.shape[0]):
        x[i][0]=vertices[Na[i]][0]
        x[i][1] = vertices[i][0]
        x[i][2] = vertices[Nb[i]][0]
        y[i][0] = vertices[Na[i]][1]
        y[i][1] = vertices[i][1]
        y[i][2] = vertices[Nb[i]][1]
